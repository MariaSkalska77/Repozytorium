...
public class Tasks {
    ...
    private static Map<String, Employee> mapOfEmployees(Employee... employees) {
        ...
        Map<String,Employee> mapapracownikow = new HashMap<>();
        for(Employee employee : employees){
            mapapracownikow.put(employee.getName() + employee.getName(), employee);
        }
        return mapapracownikow;

//        for(int i = 0; i <employees.length; i++){//iteracja po tablicy employee
//            String name = employees[i].getName();//wyciagamy imie z pracownika
//            String surname = employees[i].getSurname(); //wyciagamy nazwisko
//            employees
//        }
    }
    /**
    ...
    private static TreeMap<String, Employee> treeMapOfEmployees(Map<String, Employee> employeeMap) {
        TreeMap<String, Employee> treeMap = new TreeMap<>((o1, o2) -> -o1.compareTo(o2));//lambda,nadpisujemy metode compare, jest minus zeby pokazalo od z do a, w odwr kolejnosci
        treeMap.putAll(employeeMap);//wypelniamy mape mapa
        System.out.println(treeMap.subMap("B", "A"));
        return treeMap;
    }
    ...
    private static TreeMap<Double, Employee> treeMapOfEmployeesSalary(Employee... employees) {
        TreeMap<Double, Employee> employeeTreeMap = new TreeMap<>();
        for (Employee employee : employees) {
           double salary =  employee.getSalary();
           employeeTreeMap.put(salary, employee);

        }
        System.out.println("najnisze" + employeeTreeMap.get(employeeTreeMap.firstKey()));
        System.out.println("najwyzsze" + employeeTreeMap.get(employeeTreeMap.lastKey()));
